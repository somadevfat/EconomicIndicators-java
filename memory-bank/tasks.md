...
3.  **[ ] データインポート機能実装**
    ...
    *   [x] 時間帯ベースでの経済指標とボラティリティの紐付けロジック (`VolatilityLinkingService.java`)
        *   [x] JST時刻から該当する時間帯区分を特定する処理
        *   [x] 該当する時間帯のボラティリティデータを検索する処理
        *   [x] `EconomicIndicator`エンティティに`linkedVolatility`を設定して保存する処理
    *   [x] 5年分履歴データ一括インポート機能
        *   [ ] 大容量JSONファイルの効率的な処理方法検討（ストリーム処理など）
4.  **[x] 統計分析機能実装**
    *   [x] 5年分平均ボラティリティ計算ロジック (通貨ペア別、時間帯別) (※部分的に AverageVolatilityService として実装済み。本格的な5年分の集計と永続化は別途)
    *   [x] 地合い3分類アルゴリズム実装 (「大」「中」「小」の3分類、具体的な閾値設定) (※MarketAnalysisService として実装。閾値・係数は要調整、平均・標準偏差はダミー値)
    *   [x] 直近2回データ取得ロジック (同一指標名、同一通貨ペアの過去データを新しい順に2件取得) (※EconomicIndicatorRepository/Service に実装。通貨ペアの考慮は暫定的に指標名のみ)
    *   [x] 地合い判定ロジック (直近2回の実績値の平均と、5年分平均ボラティリティを比較し、ずれ具合で判定) (※MarketAnalysisService として実装。平均ボラティリティ比較部分は未実装、センチメント判定ロジックは実装)
5.  **[ ] REST APIエンドポイント実装** (CRUD操作含む - `projectbrief.md` 3.1.4参照)
    *   [x] `EconomicIndicatorController`
        *   [x] `POST /api/indicators`: 経済指標作成
        *   [x] `GET /api/indicators`: 経済指標一覧取得
        *   [x] `GET /api/indicators/{id}`: 経済指標詳細取得
        *   [x] `PUT /api/indicators/{id}`: 経済指標更新
        *   [x] `DELETE /api/indicators/{id}`: 経済指標削除
        *   [x] `POST /api/indicators/import`: 経済指標JSONインポート (既存タスクと統合)
    *   [x] `VolatilityDataController`
        *   [x] `POST /api/volatility`: ボラティリティデータ作成
        *   [x] `GET /api/volatility`: ボラティリティデータ一覧取得
        *   [x] `GET /api/volatility/{id}`: ボラティリティデータ詳細取得
        *   [x] `PUT /api/volatility/{id}`: ボラティリティデータ更新
        *   [x] `DELETE /api/volatility/{id}`: ボラティリティデータ削除
        *   [x] `POST /api/volatility/import`: ZigZagボラティリティJSONインポート (既存タスクと統合)
    *   [x] `AnalysisController`
        *   [x] `GET /api/analysis/summary`: 統計分析結果（地合い分類、直近2回実績、地合い判定）取得 (指標ごと)
6.  **[ ] セキュリティと品質管理**
